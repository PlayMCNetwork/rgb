...
const savedColors = ['FF6600', 'FFCD00', getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor()];
const presets = {
  1: {
...
function updateOutputText(event) {
  ...
  if (format.outputPrefix) {
    ...
    if (nickName.value) {
      ...
      if (!nickName.value.match(letters)) nickName.value = 'PlayMC Network';
    }
  }

  let newNick = nickName.value
  if (!newNick) {
    newNick = 'Digite alguma coisa!'
  }
